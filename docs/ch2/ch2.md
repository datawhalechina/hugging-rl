...
$$
\begin{equation}
\mathbb{E}[\frac{1}{H}\sum\_{t=1}^{H}\mathbb{E}\_{s\_t\sim f^t\_{\pi^{E}}}[\mathbb{E}\_{a\sim\pi\_t^{BC}(.\vert s\_t)}[\mathbb{1}(a\neq\pi\_t^{E}(s_t))]]]\le min\{1,\frac{\vert\mathcal{S}\vert}{N}\}\tag{2.6}
\end{equation}
$$
